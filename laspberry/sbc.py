...
def main(args): # 핀 명명 방식 설정
    ...
    try:
        th1=threading.Thread(target=servo, args=(servo_PWM,))
        th2=threading.Thread(target=picture,args=(cam,))
        th3=threading.Thread(target=buz,args=(passiveBuzzer,))
        th1.start()
        th2.start()
        th3.start()
        th1.join()
